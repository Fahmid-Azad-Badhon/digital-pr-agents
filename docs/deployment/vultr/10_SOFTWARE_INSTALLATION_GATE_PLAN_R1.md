...
| 3 | Node.js installation | Install Node.js 20 LTS (minimum 18.17+) | `node --version`, `npm --version` |
...
| 10 | Build/start validation | `npm ci`, `npm run build`, `npm start` on localhost (`next start -p 3002 -H 127.0.0.1`) | Build exit 0, app responds on 127.0.0.1:3002 |
