...
// =============================================================================
// Zod Guardrail - Schema Enforcement
// Prevents cascading failures when stage outputs are malformed
// =============================================================================

import { z } from 'zod'

// S4 Verified Finding Schema
...
// =============================================================================
// Automated Traceability (Paper Trail)
// Saves trace objects for debugging
// =============================================================================

const TRACE_DIR = 'D:\\Codex Folder\\digital-pr-agents\\dashboard\\traces'
...
export function saveTrace(trace: TraceObject): void {
  try {
    const fs = require('fs')
    const path = require('path')
    ...
    const filename = `trace-${trace.campaignId}-${trace.stage}-${Date.now()}.json`
    const filepath = path.join(TRACE_DIR, filename)
    
    // Truncate long strings for file size
    const truncatedTrace = {
      ...trace,
      rawInput: trace.rawInput.substring(0, 5000),
      rawOutput: trace.rawOutput.substring(0, 10000)
    }
    
    fs.writeFileSync(filepath, JSON.stringify(truncatedTrace, null, 2))
    console.log(`[Trace] Saved: ${filename}`)
  } catch (e) {
    console.log(`[Trace] Failed to save:`, e)
  }
}

export function createTraceWrapper<T>(
  campaignId: string,
  stage: string,
  model: string,
  fn: () => Promise<T>
  ...
  return async () => {
    try {
      ...
      saveTrace({
        ...
        model,
        promptVersion,
        rawInput: '', // Would need to capture this separately
        rawOutput: typeof result === 'string' ? result.substring(0, 5000) : JSON.stringify(result).substring(0, 5000),
        latency: latencyMs,
        timestamp: new Date().toISOString(),
        success: true
      })
